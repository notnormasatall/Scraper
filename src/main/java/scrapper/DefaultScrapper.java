...
public class DefaultScrapper implements Scrapper{
    ...
    private static double getBeds(Document doc) {
        String priceText = doc.selectFirst(BEDS_SELECTOR).text().replaceAll("[^0-9,]", "");
        return Double.parseDouble(priceText);
    }

    private static double getBaths(Document doc) {
        String priceText = doc.selectFirst(BATHS_SELECTOR).text().replaceAll("[^0-9,]", "");
        return Double.parseDouble(priceText);
    }

    private static double getGarages(Document doc) {
        String priceText = doc.selectFirst(GARAGE_SELECTOR).text().replaceAll("[^0-9,]", "");
        return Double.parseDouble(priceText);
    }
}
